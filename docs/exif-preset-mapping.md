...
3. When applying metadata, **`mergePayloads`** loads the selected Camera, Lens, Author, and Film presets and merges their JSON payloads in this order: **Camera → Lens → Author → Film**. If the same tag appears in more than one preset, **later categories win** (last write wins). Exception: when the effective Camera preset has a **fixed lens** (`locks_lens` / `lens_system: fixed`), the renderer passes no Lens preset to the merge (`lensPresetIdForWrite` in `src/shared/lensFilter.ts`), so the camera’s integrated lens payload is the only lens payload written.
...
| `LensModel`         | ✓ (fixed lens only)                              | ✓    |           |        | UI: “Lens Model”; legacy `LensID` in old preset JSON is migrated to `LensModel` on load when model was empty. On **write**, **`Lens`** and **`LensID`** are derived from **`LensModel`** (not stored in preset JSON; see [Derived tags at write](#derived-tags-at-write)) |
...
| --------- | ------ | -------------- |
| `Lens`    | Same string as merged **`LensModel`** when that key is in the write payload; cleared when **`LensModel`** is empty (e.g. Lens **Remove**) | `syncLensTagsFromLensModel` in `src/shared/lensWriteTags.ts`; applied in `sanitizeWritePayload` (`src/main/exifCore/pure.ts`) and `prepareWritePayloadForExif` (`src/shared/copyrightFormat.ts`) |
| `LensID`  | Same string as merged **`LensModel`** when that key is in the write payload; cleared when **`LensModel`** is empty (e.g. Lens **Remove**) | `syncLensTagsFromLensModel` in `src/shared/lensWriteTags.ts`; applied in `sanitizeWritePayload` (`src/main/exifCore/pure.ts`) and `prepareWritePayloadForExif` (`src/shared/copyrightFormat.ts`) |

If **`LensModel`** is not in the write payload at all, **`Lens`** and **`LensID`** are left unchanged on the file.
