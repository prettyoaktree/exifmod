...
| `ready` | **Green** | Closed | Yes | Ollama server is reachable; AI describe can be attempted when selection allows. Model-specific failures (missing model, model error, bad response) surface from the describe action rather than making the server status appear down. The panel can include **Show system prompt** (editable describe instructions; must include `{{MAX_DESC_BYTES}}`; saved via main). A short hint explains the placeholder; keep custom text free of long sample JSON models might copy verbatim. | — |
